...
public class MockStrutsTestCase {
  ...
  /**
   * Constructor that takes test name parameter.
   */
...
//        super(testName);
  ...
  private void init() {
    if (!isInitialized)
      Assert.fail("You are overriding the setUp() method without calling super.setUp().  You must call the superclass setUp() method in your TestCase subclass to ensure proper initialization.");
  }
...
//                fail(message);
  ...
  public void setActionServlet(ActionServlet servlet) {
    ...
    init();
    if (servlet == null)
      Assert.fail("Cannot set ActionServlet to null");
  ...
  public void setServletConfigFile(String pathname) {
    ...
    try {
      ...
      if (input == null)
        Assert.fail("Invalid pathname: " + pathname);
      digester.parse(input);
      input.close();
    } catch (Exception e) {
      Assert.fail("Received an exception while loading web.xml - " + e.getClass() + " : " + e.getMessage());
    }
  ...
  public void verifyForwardPath(String forwardPath) throws Exception {
    ...
    if (actualForward == null) {
      ...
      Assert.fail("Was expecting '" + forwardPath + "' but it appears the Action has tried to return an ActionForward that is not mapped correctly.");
    }
    if (logger.isDebugEnabled()) {
      logger.debug("expected forward = '" + forwardPath + "' - actual forward = '" + actualForward + "'");
    }
    if (!(actualForward.equals(forwardPath)))
      Assert.fail("was expecting '" + forwardPath + "' but received '" + actualForward + "'");
    if (logger.isDebugEnabled())
      logger.debug("Exiting");
  }
